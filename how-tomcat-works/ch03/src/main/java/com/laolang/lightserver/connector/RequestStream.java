package com.laolang.lightserver.connector;
...
import com.laolang.lightserver.util.StringManager;

/**
 * Convenience implementation of <b>ServletInputStream</b> that works with the standard
 * implementations of <b>Request</b>. If the content length has been set on our associated
 * Request, this implementation will enforce not reading more than that many bytes on the
 * underlying stream.
 *
...
public class RequestStream extends ServletInputStream {
     ...
     * attempt to read from this stream will throw an IOException. If a content length has
     * been set but not all of the bytes have yet been consumed, the remaining bytes will
     * be swallowed.
     */
    ...
    /**
     * Read some number of bytes from the input stream, and store them into the buffer
     * array b. The number of bytes actually read is returned as an integer. This method
     * blocks until input data is available, end of file is detected, or an exception is
     * thrown.
    ...
    /**
     * Read up to <code>len</code> bytes of data from the input stream into an array of
     * bytes. An attempt is made to read as many as <code>len</code> bytes, but a smaller
     * number may be read, possibly zero. The number of bytes actually read is returned as
